...
describe('map', () => {
    ...
    returns a new array with all the values in
     the array passed to the function doubled. */
    xit('doubleValues', (done) => {
    ...
    returns a new array with each value
     multiplied by the index it is currently at in the array. */
    xit('valTimesIndex', (done) => {
    ...
    xit('extractKey', (done) => {
        ...
        assert(_.isEqual(extractKey([{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'},
                {name: 'Colt'}], 'name'),
            ['Elie', 'Tim', 'Matt', 'Colt']));
        done();
    });
    ...
    objects and returns a new array with the
     value of the key with a name of "first" and the
     value of a key with the name of  "last" in each object,
      concatenated together with a space. */
    xit('extractFullName', (done) => {
